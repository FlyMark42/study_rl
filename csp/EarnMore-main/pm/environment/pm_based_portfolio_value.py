...
class EnvironmentPV(gym.Env):
    ...
    def step(self, action: np.array = None):
        ...
        self.day += 1
        done = self.day >= self.num_days - 1  # True if episode ends
        ...
        # old value
        old_portfolio_value = self.protfolio_value  # V_{t-1}

        # weights[0] is the cash, weights[1:] is the stocks
        new_portfolio_value = (1 - weights[0]) * old_portfolio_value * (1 + portfolio_ret) \
                         + weights[0] * old_portfolio_value  # V_t

        # 论文定义的奖励：r_t = V_t - V_{t-1}
        reward = new_portfolio_value - old_portfolio_value

        # 更新portfolio value
        self.protfolio_value = new_portfolio_value
        ...
        # ✅ Gym 0.26+ API 兼容：返回 terminated 和 truncated
        terminated = done
        truncated = False
