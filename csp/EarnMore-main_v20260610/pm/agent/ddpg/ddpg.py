...
class AgentDDPG():
    def __init__(self,
        ...
        self.forward_noise_action = forward_action_wrapper(self.act_target.get_action_noise, method=action_wrapper_method)
    ...
    def update_net(self, buffer: ReplayBuffer) -> dict:
        ...
        for _ in tqdm(range(update_times),
            ...
            action_pg = self.forward_action(state)  # policy gradient
    ...
    def get_obj_critic_raw(self, buffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        with torch.no_grad():
            ...
            next_actions = self.forward_noise_action(x = next_states, action_std = self.policy_noise_std)

            next_q_values = self.cri_target(next_states, next_actions)  # next q_values
            q_labels = rewards + (1.0 - dones) * self.gamma * next_q_values
