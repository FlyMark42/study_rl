...
class ParameterSpaceManager:
    ...
    def __init__(self, config_path="param_spaces.json"):
...
class BayesianOptimizer:
    ...
    def bayesian_search(self, template_name, max_iterations=50, env_type=None,
        ...
        # 生成实验配置文件
        self._generate_experiment_configs_from_bayesian(result, param_names, search_id, template_name)
    ...
    def resume_from_search(self, search_id, template_name):
        """从已有搜索恢复并继续优化"""
        ...
        # 生成新的实验配置
        new_search_id = self._generate_next_experiments(search_id, template_name, new_results)
    ...
    def _extract_asr_from_experiment(self, experiment_name):
        """从实验中提取ASR结果"""
        metrics_path = os.path.join("experiments", experiment_name, "logs", "training_metrics.csv")
...
def main():
    ...
    if args.command == 'detect-environment':
        ...
        optimizer = BayesianOptimizer(param_manager)
        result = optimizer.resume_from_search(args.search_id, args.template)
        if result:
