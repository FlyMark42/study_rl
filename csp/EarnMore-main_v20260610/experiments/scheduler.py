...
g_strRoot="J:\\python_code\\EarnMore-main2"
g_strTrainCmd="J:\\python_code\\EarnMore-main2\\tools\\train.py"
...
class TaskQueue:
    ...
    def _init_status(self, exp_dir, exp_name):
        ...
        # 基于config.py文件或已有训练结果判断初始状态
        if os.path.exists(config_file) or os.path.exists(os.path.join(exp_dir, "best.pth")):
...
def main():
    ...
    parser.add_argument("--traincmd", type=str, default=None, help="训练脚本路径，默认使用 tools/train.py")
    ...
    global g_strTrainCmd
    if getattr(args, 'traincmd', None):
        g_strTrainCmd = args.traincmd
        print("使用自定义训练命令: {}".format(g_strTrainCmd))
