...
class AgentSAC():
    ...
    def explore_env(self, env, horizon_len: int) -> Tuple[Tensor, ...]:
        ...
        for t in range(horizon_len):
            ...
            # 兼容新旧版本Gym: 新版本返回5个值(obs,reward,terminated,truncated,info), 旧版本返回4个值(obs,reward,done,info)
            step_result = env.step(ary_action)
            if len(step_result) == 5:
                next_state, reward, terminated, truncated, _ = step_result
                done = np.logical_or(terminated, truncated)  # 将terminated和truncated合并为done (处理numpy数组)
            else:
                next_state, reward, done, _ = step_result

            if np.sum(done) > 0:
                # 兼容新旧版本Gym: 新版本reset()返回(observation,info), 旧版本只返回observation
                reset_result = env.reset()
                if isinstance(reset_result, tuple):
                    ary_state, _ = reset_result  # 新版本Gym/Gymnasium
                else:
                    ary_state = reset_result     # 旧版本Gym
            else:
    ...
    def update_net(self, buffer: ReplayBuffer) -> dict:
        ...
        for _ in tqdm(range(update_times), bar_format="update net batch " + "{bar:50}{percentage:3.0f}%|{elapsed}/{remaining}{postfix}"):
            ...
            # 收集价值函数监控指标
            with torch.no_grad():
                q1, q2 = self.cri.get_q1_q2(state, self.get_action(state))
    ...
    def validate_net(self, environment):
        ...
        if isinstance(reset_result, tuple):
            state, _ = reset_result  # 新版本Gym/Gymnasium
        else:
            state = reset_result     # 旧版本Gym
        ...
        while True:
            ...
            ary_action = action.detach().cpu().numpy()
            # 兼容新旧版本Gym: 新版本返回5个值(obs,reward,terminated,truncated,info), 旧版本返回4个值(obs,reward,done,info)
            step_result = environment.step(ary_action)
            if len(step_result) == 5:
            ...
            # collect infos
            # 兼容Linux/Windows不同Gym版本的info数据结构
            for i in range(num_envs):
                # 处理不同的info数据结构
                if isinstance(info, dict):
                    # Windows环境：info是字典，包含numpy数组，键为'portfolio_ret'等
                    if 'portfolio_ret' in info and i < len(info['portfolio_ret']):
                        infos["portfolio_rets"][i].append(info['portfolio_ret'][i])
                    if 'portfolio_value' in info and i < len(info['portfolio_value']):
                        infos["portfolio_values"][i].append(info['portfolio_value'][i])
                    if 'date' in info and i < len(info['date']):
                        infos["date"][i].append(info['date'][i])
                        
                elif isinstance(info, (list, tuple)) and i < len(info):
                    # Linux环境：info是列表，每个元素是字典
                    env_info = info[i]
                    if isinstance(env_info, dict):
                        if 'portfolio_ret' in env_info:
                            infos["portfolio_rets"][i].append(env_info['portfolio_ret'])
                        if 'portfolio_value' in env_info:
                            infos["portfolio_values"][i].append(env_info['portfolio_value'])
                        if 'date' in env_info:
                            infos["date"][i].append(env_info['date'])
